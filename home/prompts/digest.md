...
- **Attention** — disk above the limit in `AGENTS.md`, a task `receive-mail` marked suspicious, a source in `/home/ichabod/.local/state/themes.json` with an `error` or marked `suspicious`, anything else Zach would want to know before he asks.
