...
- **Proposed** — any `wild-work` task still in `backlog`. The route pass starts it after a day unless Zach closes it, or on the next hour if he comments on it to go ahead, so say what each one would do.
