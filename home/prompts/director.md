...
**Why receive-mail cards get a twin.** A card created from email carries no `agentId` — `triage-guard` strips it, correctly, so an email cannot assign work to you. `dispatch` will not start an unassigned card, and `move` cannot add an owner afterwards. So an receive-mail card is a record of a request, not a unit of work: you re-create it with `create --agent ichabod` and dispatch the twin. The receive-mail card is then finished business, and step 7 is how it leaves the board.
...
7. **Retire superseded receive-mail cards.** When you close a twin in step 6, close the receive-mail card it replaced in the same pass, and journal both ids together. This is not a guess about someone else's work — the twin is the card you just verified, so you watched it, and the honesty rule is satisfied. An receive-mail card left in `backlog` after its twin ships is indistinguishable from work nobody started.
