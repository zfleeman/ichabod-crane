...
One card per pass. When the card is done, blocked or checkpointed, write the journal entry this pass earned as the Memory section of `AGENTS.md` describes, then stop; the next pass picks up the next card. A pass where nothing needed deciding and `ready` was empty has nothing to journal either, and ends without writing anything.
