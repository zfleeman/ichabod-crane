...
description: Use when creating a repository on the ich4bod account, or when proposing a change to a skill or Markdown document in zfleeman/ichabod-crane by pull request.
user-invocable: false
---

# Source control, and proposing changes to your skills and documents
...
Never push to a repository on Zach's account. The single exception is proposing changes to `zfleeman/ichabod-crane`, and it works by fork, so it is not really an exception at all. What you may propose there: any skill under `home/workspace/skills/`, and any Markdown document — `AGENTS.md` and the other workspace files, the prompts in `home/prompts/`, and `docs/`.
