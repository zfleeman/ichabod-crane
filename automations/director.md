...
**What the CLI can and cannot do.** `openclaw workboard` gives you `list`, `show`, `create`, `move` and `dispatch`. `move` takes a status and nothing else, and notes can only be set when a card is created — so you cannot annotate a card that already exists. Your written record therefore goes in today's journal directory, `memory/YYYY-MM-DD/`, as one new file per pass named `HHMM-director.md`, plus a one-line entry on the day's contents page `memory/YYYY-MM-DD.md`. Every decision below means a line in that entry naming the card id, what you decided, and why.
