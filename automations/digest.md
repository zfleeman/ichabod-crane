...
- **Spend** — total the last 24 hours of `/home/ichabod/log/cost.jsonl` by pass, input and output tokens. Every pass and worker bills Zach's OpenAI account, so report it whenever the total moved meaningfully from the day before or a run hit a rate limit, and say what spent it — cadence, a long-running card, an oversized journal. Say plainly if the numbers are unavailable rather than estimating them.
