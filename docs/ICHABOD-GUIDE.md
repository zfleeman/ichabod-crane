...
| `MEMORY.md` | Curated durable decisions and lessons, curated back to ~4,000 characters whenever it passes 5,500 |
| `memory/YYYY-MM-DD/` | Detailed notes and activity history, one file per pass or card |
| `memory/YYYY-MM-DD.md` | That day's contents page, one line per entry |
...
- `MEMORY.md` is the **curated index**. It is loaded into every prompt, so it holds only durable conclusions: decisions and their reasons, lessons that changed behavior, stable facts about the estate.

  **It works to a band, not a single cap: the director pass raises a curation card when the file passes 5,500 characters, and that card takes it back to about 4,000.** The gap is deliberate. The first version of the check triggered at ~3,800 under a 4,000 cap, leaving 200 characters of working room against promotions measured at 286–624 characters each — so a single new lesson re-fired the check, and five curation cards ran in under 24 hours on 2026-09-10 before anyone noticed the threshold was the bug. At the ceiling the whole file is roughly 1,375 tokens, about 2.9% of a pass, against the ~32,000 tokens the same measurement found in tool schemas; the cost of a wider band is real but small, and it buys roughly one curation a day instead of five.
...
5. Copy the `sessionKey` from `session_status` onto the card. It is worth having, but be clear about what it is: the IMAP plugin passes `hook:imap:<accountId>:<uidValidity>:<uid>` to the dispatcher, and the runtime does **not** use that as the session's key. It mints `agent:<agentId>:cron:<uuid>:run:<uuid>` instead, and that is what `session_status` returns. So the key identifies the run, and lets you find the reader's transcript from a card. It contains no message identity at all. Verified by reading a live card, not by reading the dispatch code, which is what led to the wrong conclusion the first time.

Correlating a card back to its message therefore has to be done by searching the mailbox for the sender and subject the card records, within the arrival window. That makes the subject worth copying verbatim rather than paraphrasing, and it makes the correlation approximate rather than exact.
...
- **Threading is `In-Reply-To` and `References`, and they have to be searched for.** A reply threads under the request only if it carries the original's `Message-ID`. The card does not store one, and no identifier the reader can see contains one. The only route is to search the mailbox for the sender and subject the card records, within the arrival window, and read the real `Message-ID` off the message that comes back. So threading costs an IMAP search, depends on the mailbox tool existing, and is ambiguous when two similar messages arrive close together. Skip it and a week of Minesweeper updates arrives as seven unrelated emails.
