...
**A pass is one prompt file in `home/prompts/` and one line in `home/crontab`.** `run` gives it a lock, a timeout, a JSON log, and a cost line. The `work` pass tidies the board, writes acceptance criteria for new cards, and works one card, all in one run, so an emailed request can start within the hour and an hour costs one preamble instead of two. The shipped schedule lives in `/etc/cron.d`, so Ichabod's own crontab stays his, and `make deploy` never installs it, so a deploy cannot re-enable a pass a kill switch turned off.
